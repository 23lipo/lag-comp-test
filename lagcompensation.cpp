...
{
	for ( int i = 1; i <= game.m_globals( )->m_maxclients; i++ ) 
	{
		auto e = static_cast< player_t * >( game.m_entitylist( )->GetClientEntity( i ) );
		...
		case FRAME_RENDER_START:
			*( int* ) ( ( uintptr_t ) e + 0xA30 ) = game.m_globals( )->m_framecount;
...
{
	last_eye_positions.push_front( context.m_local->get_eye_pos( ) );
	if ( last_eye_positions.size( ) > 128 )
		last_eye_positions.pop_back( );

	auto nci = game.m_engine( )->GetNetChannelInfo( );
	if ( !nci )
		return;
...
void player_record_t::log( ) 
{
	...
	if ( game.m_globals( )->m_curtime - m_track.back( ).m_curtime > 1.f )
		m_track.pop_back( );
}
...
std::deque< tickrecord_t > player_record_t::get_valid_track( ) 
{
	auto delta_time = [ & ]( float simulation_time ) -> float 
	{
		...
		return fabs( correct - ( game.m_globals( )->m_curtime - simulation_time ) );
	};
...
bool lagcompensation::continue_loop( int id, player_t * e, ClientFrameStage_t stage ) 
{
	...
	case FRAME_NET_UPDATE_POSTDATAUPDATE_END:
		if ( e->m_iHealth( ) <= 0 || e->m_iTeamNum( ) == context.m_local->m_iTeamNum( ) ) 
		{
...
{
	if ( !game.m_clientstate( ) )
		return;

	auto net_channel = game.m_clientstate( )->m_NetChannel;
	...
	if ( net_channel ) 
	{
		if ( net_channel->m_nInSequenceNr > m_last_incoming_sequence_number ) 
		{
			...
			sequences.push_front( incoming_sequence_t( net_channel->m_nInReliableState, net_channel->m_nOutReliableState, net_channel->m_nInSequenceNr, game.m_globals( )->m_realtime ) );
		}
